  ...
  try {
    ...
    if (!cart) {
      ...
      if (existItem) {
        ...
        cartItems.find((x) => x.productId === item.productId)!.qty = existItem.qty + 1;
      } else {
...
export async function removeItemFromCart(productId: string) {
  try {
    ...
    if (exist.qty === 1) {
      ...
      cartItems.find((x) => x.productId === productId)!.qty = exist.qty - 1;
    }
    ...
    return {
      success: true,
      message: `${product.name}  ${isRemoveAllItems ? 'removed from' : 'updated in'} cart successfully`,
    };
  } catch (error) {
    return { success: false, message: formatError(error) };
  }
